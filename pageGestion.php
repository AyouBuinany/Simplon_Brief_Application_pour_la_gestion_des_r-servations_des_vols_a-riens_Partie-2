...
if($flight_list->showAllFlights($query)){
$result=$flight_list->showAllFlights($query);
$row=mysqli_num_rows($result);
}
if(isset($_SESSION["user"])){
    $messageSession="
        <h3 style='margin: auto; text-align: center; font-size:27px'> Welcome Admin <span style='color:#f07d43'>" .$_SESSION["user"][1]. "  </span>To Morocco AirLines";
        "</h3>" ;
                         }
/*user*/
$admin= new users();
...
//cancel
    ...
    $idFlight=0;
     if(isset($_GET["cancel"]) && !empty($_GET["cancel"])){
        $idFlight=  $_GET["cancel"];
    }
   $user=(isset($_SESSION["user"])) ? $_SESSION["user"][0] : NULL ;
   if($idFlight!=0 && $user!=NULL){
    if($admin->cancel($user,$idFlight))
...
<body>
    ...
    <div class=" header-container">
        <div class="inner-container">
            ...
            <div id="menu">
                <ul id="fixed-ul">
                    <li><a href="profileAdmin.php"><img src="./img/profile.png" alt="user" width="40"></a></li>
                </ul>
            </div>
        </div>
    ...
    <div class="col-md-10  reservation_table" style="margin: auto;">
        ...
        <?php while($vol=mysqli_fetch_array($result)): ?>
            <tr>
                ...
                <td ><?= $vol['seats']; ?></td>
                <td>
                <a href="pageGestion.php?show=<?= $vol['id'];  ?>" class="badge badge-primary p-3" id="reserver">Show</a>
                <a href="pageGestion.php?cancel=<?= $vol['id'];  ?>" class="badge badge-primary p-3" id="reserver">Cancel</a>
                </td>
            </tr>
            <?php endwhile; ?>
        <?php endif; ?>
...
<a href="pageGestion.php" class="close"   onclick="document.getElementById('alert').style.display='none';">
